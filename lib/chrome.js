...
var ChromeObject = function() {
    ...
    this.setText = function(selector, value, repeat, searchIndex) {
        var s = value,
            i = 0,
            searchIndex = (typeof searchIndex !== "undefined" ? searchIndex : 0),
            repeat = (typeof repeat !== "undefined" ? repeat : 1)
        ...
        while (i < repeat) {
            if (searchIndex > 0) {
                this.evaluate('Object.values(__getDocument().querySelectorAll("' + selector + '"))[' + searchIndex + '].innerText = ' + this.__escape(s));
            } else {
                this.evaluate('__getDocument().querySelector("' + selector + '").innerText = ' + this.__escape(s));
            }
            i++;
        }
    };
    ...
    this.vMouseClick = function(x, y) {
        UseObject("WelsonJS.ProcessControl", function(pc) {
            pc.SendClick(this.pageId.substring(0, 6), this.baseScreenX + x, this.baseScreenY + y, 1);
        });
    };

    this.vSendKeys = function(s) {
        UseObject("WelsonJS.ProcessControl", function(pc) {
            pc.SendKeys(this.pageId.substring(0, 6), s);
        });
    };
...
exports.VERSIONINFO = "Chrome Debugging Protocol Interface (chrome.js) version 0.5.6";
